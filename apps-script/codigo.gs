...
const LUGARES_HEADERS = [
  "Tipo", "Nombre", "Ubicacion", "Telefono", "Insumo", "Categoria", "Estado", "Actualizado", "Latitud", "Longitud"
];
...
function doGet(e) {
  try {
    ...
    if (accion === "buscar_familiar") return proxyBuscarFamiliar(params);
...
function abrirSpreadsheet() {
  try {
    ...
    // No está en un script enlazado, se abre por ID
  }
...
function construirLugares() {
  ...
  rows.forEach(function (row) {
    ...
    const latitud = texto(valorPorCabecera(row, ["Latitud", "latitud", "lat"]));
    const longitud = texto(valorPorCabecera(row, ["Longitud", "longitud", "lon", "lng"]));
    ...
    if (latitud) lugaresMap[key].latitud = latitud;
    if (longitud) lugaresMap[key].longitud = longitud;
...
function registrarLugar(payload) {
  ...
  const obj = {
    "Tipo": tipo,
    "Nombre": nombre,
    "Ubicacion": texto(payload.ubicacion || payload.Ubicacion),
    "Telefono": texto(payload.telefono || payload.Telefono),
    "Insumo": insumo,
    "Categoria": texto(payload.categoria || payload.Categoria) || "Otros",
    "Estado": esNecesita(estado) ? "Necesita" : "Tiene disponible",
    "Actualizado": new Date(),
    "Latitud": texto(payload.latitud || payload.Latitud || payload.lat),
    "Longitud": texto(payload.longitud || payload.Longitud || payload.lon || payload.lng)
  };

  anexarObjeto(LUGARES_SHEET, LUGARES_HEADERS, obj);
...
// -- PROXY DE BÚSQUEDA DE FAMILIARES ----------------------------------------
function proxyBuscarFamiliar(params) {
  const query = texto(params.query || params.q || "");
  if (!query) {
    return jsonResponse({ encontrado: false, resultados: [] });
  }

  const scriptProperties = PropertiesService.getScriptProperties();
  const webhookUrl = scriptProperties.getProperty("N8N_WEBHOOK_URL");
  const webhookToken = scriptProperties.getProperty("N8N_WEBHOOK_TOKEN");

  if (!webhookUrl) {
    return jsonResponse({
      error: "El administrador no ha configurado N8N_WEBHOOK_URL en las propiedades del script.",
      encontrado: false,
      resultados: []
    }, 400);
  }
  ...
  try {
    ...
    if (code < 200 || code >= 300) {
      return jsonResponse({
        error: "Error del servidor de búsqueda (" + code + "): " + text,
        encontrado: false,
        resultados: []
      }, code);
    }
    ...
    return jsonResponse({
      encontrado: data.encontrado === true || (data.resultados && data.resultados.length > 0),
      resultados: data.resultados || []
    });
  } catch (err) {
    return jsonResponse({
      error: "Error de red al consultar el servidor de búsqueda: " + errorMessage(err),
      encontrado: false,
      resultados: []
    }, 500);
  }
